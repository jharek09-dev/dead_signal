...
| **Expansion v2 (Narrative Depth)** | 🚧 M-EXP stage 3 started | P1 | Canon ✅; doc propagation ✅; content/code **in progress** (M-EXP, §11) — Echoes + all five region sub-stories + U1 + S1 + prologue second act + loc/a11y deltas logged (Phase 3 59 nodes, ≈2×); final regression bar (§6.1) is the last criterion |
...
| 1.9 | 2026-07-10 | **M-EXP — S1 truth-by-assembly built.** A region's truth now resolves only once its 2–3 supporting evidence pieces (existing Case-File keys) are collected AND its room is reached; the Case File assembles evidence → ∴ deduction → TRUTH (new `assembly_note` card) instead of the room dumping it. Reach a truth room early → a withholding beat naming what's missing; collect the last piece elsewhere → a nudge back to the room. `TRUTH_PIECES`: you = director/admit/procedure · signal = loop/Kim-refused · project_haven = department/authorized · outbreak = breach/lab/patient-zero. Headless: build clean (995 modules), soft-lock trace PASS (all pieces reachable, none behind their own room), finale 4-truth + Annex 2-truth gates intact, no new save field (pieces=raisedQuestions, truths=discoveredTruths). Canon audit **PASS**: withholding/assembly copy spoiler-safe, no finale leak; truth ids / gates / both endings **unchanged**. Comms + City Hall now force a one-node investigative detour; Mercy + Annex assemble on the direct path. STORY §6 wording reconciled (deduction, not contradiction). |
| 1.10b | 2026-07-10 | **M-EXP — loc/a11y deltas logged.** `docs/localization/LOCALIZATION.md` v1.2 + `docs/accessibility/ACCESSIBILITY.md` v1.2: flipped the Expansion-v2 *content* flags PLANNED→BUILT and recorded the measured string/content delta (Phase 3 34→59; 14 Echoes; ~25 node string-sets + ~25 facts + U1 upgrade; S1 assembly strings; prologue midpoint ×3 + shelter). Loc: the ~2× is real and authored **inline** — externalization (M-LOC0) is now a single full extraction; per-language Phase-3 effort re-baselined to 59 nodes. A11y: confirmed the Echo cue's visual twin (ECHO RECOVERED text card + `sigFlicker`), folded it + the S1 `assembly_note` flash into the photosensitivity audit, widened §10 content/trauma notes for the specific landed beats, brought the new board content under the SR/keyboard/contrast/text-speed bar. The loc/a11y **work** (M-LOC0 / M-A11Y option set + audit) stays PLANNED — only the content it must cover has landed. **M-EXP: 5 of 7 criteria done; the full regression bar (§6.1) remains.** |
| 1.10 | 2026-07-10 | **M-EXP — prologue second act built (P1–P3).** **P1** — a real midpoint per route (new choiceless `MIDPOINT_BEAT`, modelled on the calm beat): a trace of others who ran before you (child-token varied per route — shoe / stuffed rabbit / small backpack — the arrows that stop, `WE'RE GETTING OUT`), seeding the *unchosen* theme early, spoiler-safe. **P2** — the shelter set-piece becomes a scene: the previous occupant (blanket-shape, cold coffee), a faint wrongness in the Haven broadcast ("the voice catches… like it's reading, not remembering"), and one Ellie crack ("don't listen to that too close") that deepens KIM→ELLIE. **P3** — the midpoint + shelter Ellie lines make the guide a relationship, within the crack ladder (§3/§4). Economy UNCHANGED (the midpoint replaces the mid-leg atmo, keeping its `path_mid` drain; same 3 encounters + calm); new `midpointFired` flag persists in the save (state/ref/save/restore/reset). Zero answers, no Echoes, says "broadcast" never "the Signal." Build clean; canon audit **PASS** (spoiler discipline intact, Ellie never acts in physical space, unchosen reads as generic refugees). Prologue's crack-only discipline (§7) held. |
